#!/usr/bin/env python3
"""
バドミントン Top 10 クエリスクリプト

スクリーニングロジック: オリンピックバドミントン 金メダル 2個以上獲得
"""
import argparse
import json
import sys
...
from src.optimizations.badminton_top10 import get_top10, get_screening_logic


def main():
    parser = argparse.ArgumentParser(
        description="バドミントン Top 10（金メダル2個以上）"
    )
    parser.add_argument("--min", type=int, default=2, help="最小金メダル数")
    parser.add_argument("--gender", choices=["M", "F"], default=None, help="性別でフィルタ")
    parser.add_argument("--discipline", choices=["singles", "doubles", "mixed", "multiple"], default=None, help="種目でフィルタ")
    ...
    if args.json:
        ...
        print(f"【バドミントン Top 10】金メダル {args.min}個以上{gender_str}{disc_str}\n")
        print("スクリーニング: オリンピックバドミントン 金メダル 2個以上 | ソート: 金メダル数降順\n")

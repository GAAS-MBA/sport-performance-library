#!/usr/bin/env python3
"""
短距離走 Top 10 クエリスクリプト

スクリーニングロジック: オリンピック短距離（100m, 200m, 4x100m）金メダル 3個以上獲得
"""
import argparse
import json
import sys
...
from src.optimizations.sprint_top10 import get_top10, get_screening_logic


def main():
    parser = argparse.ArgumentParser(
        description="短距離走 Top 10（100m, 200m, 4x100m 金メダル3個以上）"
    )
    ...
    if args.json:
        ...
        print(f"【短距離走 Top 10】100m/200m/4x100m 金メダル {args.min}個以上{gender_str}\n")
        print("スクリーニング: オリンピック短距離 金メダル 3個以上 | ソート: 金メダル数降順\n")
